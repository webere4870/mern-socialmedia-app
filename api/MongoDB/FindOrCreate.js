...
async function FindOrCreate(username, password, provider, name, picture)
{
    return new Promise((resolve, reject)=>
    {
        UserSchema.findOne({_id: username}, async (err, result)=>
        {
            if(!result && !provider)
            {
                bcrypt.genSalt(10, (err, salt) => {
                    bcrypt.hash(password, salt, async function(err, hash) {
                        let newRecord = await UserSchema.create({_id: username, hash: hash, salt: salt, name: name, picture: picture, provider: "E-Web-Software", active: false, notifications: [], city: "", state: "", bio: "", reviews: [], overall: 0, saved: [], unread: [], active: false, stripe: "", subscriptions: [], availableReviews: [] })
            ...
            {
                let newRecord = await UserSchema.create({_id: username, hash: "", salt: "", picture: picture, provider: provider, name: name, active: false, notifications: false, city: "", state: "", bio: "", reviews: [], overall: 0, saved: [], unread: [], active: true, stripe: "", subscribers: [], subscriptions: [], availableReviews: []})
                newRecord.save()
                let client = container.getBlockBlobClient(username)
                let background = container.getBlockBlobClient("bg"+username)
                fs.rename("default.png", username, async (response)=>
