...
var blobService = new BlobServiceClient(process.env.AZURE_CONNECTION_STRING);
var container = blobService.getContainerClient("react-app")
container.getBlockBlobClient("react")
...
router.post("/profilePicture", ValidateJWT, upload.single('avatar'), async (req, res)=>
{
    ...
    let temp = container.getBlockBlobClient("newby.jpg")
    console.log(path.join(__dirname, "\\..\\uploads\\"+req.file.filename))
    await temp.uploadFile(path.join(__dirname, "\\..\\uploads\\"+req.file.filename))
    //console.log(path.join(__dirname, "\\..\\uploads\\"+req.file.filename))
